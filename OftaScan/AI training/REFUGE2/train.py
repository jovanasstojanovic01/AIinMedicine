...
def train_epoch(model, loader, optimizer, loss_fn, scaler, device):
    ...
    for images, masks in loop:
        ...
        # Koristimo mešovitu preciznost (AMP) radi uštede VRAM-a i bržeg treninga na GPU
        with torch.amp.autocast(device_type='cuda' if 'cuda' in device else 'cpu'):
            outputs = model(images)
            loss = loss_fn(outputs, masks)
            
        # Skaliranje gradijenata sprečava numeričke greške (underflow) zbog float16 formata
...
def validate_epoch(model, loader, loss_fn, device):
    ...
    # Isključujemo gradijente tokom evaluacije da bismo oslobodili memoriju
    with torch.no_grad():
        for images, masks in loop:
            images = images.to(device)
            masks = masks.to(device)
...
def main():
    ...
    # Odvojene transformacije: trening uključuje augmentaciju (robusnost), validacija je deterministička
    ...
    # pin_memory=True ubrzava prenos podataka sa CPU-a na GPU
    train_loader = DataLoader(train_dataset, batch_size=config.BATCH_SIZE, shuffle=True, num_workers=2, pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=config.BATCH_SIZE, shuffle=False, num_workers=2, pin_memory=True)
    
    model = RefugeUNet(in_channels=3, out_channels=2).to(config.DEVICE)
    
    # Kombinovani loss balansira debalans klasa (sitne strukture diska naspram velike pozadine)
    loss_fn = CombinedDiceBCELoss(bce_weight=0.5)
    
    # AdamW ispravno rešava L2 regularizaciju (weight decay) nezavisno od gradijenata
    ...
    for epoch in range(1, config.EPOCHS + 1):
        ...
        # Čuvamo samo model sa najmanjim validacionim gubitkom (zaštita od overfitting-a)
    ...
    # Plotovanje krive učenja za dokumentaciju
